...
| 시각 | 세션 | 동작 |
|------|------|------|
| 05:45 | `us` 시작 | 라이브 폴링 (05:50~06:00 미국 패널 방송이 있으면 포함) |
| 05:50~06:40 | `us` 녹화 | 미국 시황 방송 녹화 |
| ~07:00 | `us` 전송 | 🇺🇸 미국 전일 시황 리포트 → 텔레그램/카카오 + 옵시디안 |
| 07:50 | `kr` 시작 | 라이브 폴링 |
| 07:55~08:20 | `kr` 녹화 | 한국 시황 방송 녹화 |
| ~08:35 | `kr` 전송 | 🇰🇷 한국 당일 전망 (아침 us 리포트 연동) → 같은 옵시디안 파일에 병합 |
...
```
Windows 작업 스케줄러에 `run_local.bat us`(평일 05:45), `run_local.bat kr`(평일 07:50)를 등록하면 Actions 없이도 동일하게 동작합니다.
