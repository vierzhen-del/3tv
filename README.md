...
2. **GitHub cron 지연·드롭** — 스케줄 실행이 40~55분 늦게 시작되는 것은 물론(2026-07), 2026-08-27엔
   2~3시간 지연과 실행 자체가 아예 안 도는(드롭) 사례까지 실측됨. cron을 방송 55분 전(05:00/07:00
   KST)에 기동하고 스크립트가 방송 시작까지 대기하는 구조로 웬만한 지연은 흡수하지만, 드롭은
   흡수가 안 된다 — 그래서 **S9의 n8n이 `workflow_dispatch`를 직접 호출해 GitHub cron 큐를
   우회**하도록 이중화했다(`docs/n8n_scheduler.md`). `on: schedule`은 안전망으로 남겨둠
