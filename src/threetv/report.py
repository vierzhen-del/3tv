...
def _quote_line(q: dict) -> str:
    ...
    # 대표 기준일과 다른 항목만 자기 날짜를 병기 (market.py가 문구를 이미 완성해 둠)
    tail = f" {q['date_note']}" if q.get("date_note") else ""
...
- **종가 기준일은 섹션 제목에 한 번만** 쓰세요 (예: `💹 주요 지표 (7/24 종가 기준)`).
  ...
  ⚠️ **줄 끝 날짜 표기는 각 항목의 `date_note` 필드가 있을 때만, 그 문구를 그대로**
  붙이세요(예: `⚠️(7/16 기준)` 또는 `(8/30 기준)`). `date_note`가 없는 항목(대표
  기준일과 같음, KOSPI·KOSDAQ 포함)에는 **절대 날짜를 붙이지 마세요**.
  **`asof` 필드를 직접 비교해서 날짜 표기 여부를 스스로 판단하지 마세요** — 이미
  코드가 필요한 항목에만 `date_note`를 만들어 뒀습니다. `asof` 값 자체는 무시하세요.
