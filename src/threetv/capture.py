...
_VOD_DOWNLOAD_TIMEOUT = 3600   # 초 — 2026-08-27 실측 이후 상향(아래 docstring 참고).
                                # us-session.yml 잡 전체 예산은 150분: 다운로드에 최대
                                # 60분을 줘도 설치·분석·리포트·알림에 90분이 남는다.
...
def download_vod(
    ...
    ⚠️ 2026-08-27: 방금 올라온 VOD의 --download-sections 트리밍 다운로드가
    30분 타임아웃 안에 못 끝난 사례가 있었다(us-session run #50). "구간 seek이
    아직 안 편해서"라는 가설로 트리밍 없는 전체 다운로드를 자동 재시도하는
    로직을 넣었었는데, 같은 날 그 전체 다운로드조차 동일하게 30분에 걸려
    실패했다(run #51 — 사람이 수동으로 트리밍 없이 재실행한 것도 실패,
    로그에 진행률 정보가 전혀 없어 멈춘 건지 그냥 느린 건지도 판단 불가였음).
    즉 트리밍 자체는 원인이 아니었고, 재시도로 우회할 수 있는 문제도 아니다.
    그래서 자동 재시도는 없앴다 — 대신 (1) 타임아웃을 늘려 그냥 오래 걸리는
    경우를 흡수하고, (2) --quiet를 빼 타임아웃 시 실제 진행률/속도를 로그에
    남겨 다음에 또 느리면 원인을 바로 알 수 있게 했다.
    """
    ...
    # --quiet는 다른 호출부(get_stream_url 등, -g/-J로 파싱용 출력을 받는 곳)엔
    # 필요하지만 여기선 타임아웃 시 진행률을 보려고 빼고 --newline으로 대체한다.
    cmd = [c for c in _ytdlp_base(cookies) if c != "--quiet"] + [
        "--newline",
    ...
    try:
        ...
        lines = (e.stdout or "").strip().splitlines()
        tail = "\n".join(lines[-5:]) if lines else "(진행률 로그 없음)"
        raise CaptureError(
            f"VOD 다운로드가 {_VOD_DOWNLOAD_TIMEOUT}초 내에 끝나지 않음: {vod_url}\n"
            f"타임아웃 시점 진행 상황:\n{tail}"
        )
