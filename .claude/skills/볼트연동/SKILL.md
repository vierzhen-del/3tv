...
이 사고로 볼트에 한 번도 안 들어간 과거분(2026-07~08)은 `docs/n8n_backfill_workflow.json`
(일회성 수동 실행 워크플로)로 메웠다 — **GitHub PAT는 채팅에 새로 붙여넣지 말고**, 기존
동기화 워크플로 노드에 이미 설정된 Authorization 값/Credential을 그대로 복사해 쓴다.
백필 완료 후엔 이 워크플로를 비활성 상태로 두거나 삭제해도 무방하다(일회성 용도).
