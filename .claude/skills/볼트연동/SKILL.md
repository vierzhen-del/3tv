...
> ✅ **2026-08-11 bind mount 사고 해결됨.** 한동안 4번이 구조적으로 끊겨있어서
> (n8n 프로세스의 `/root/obsidian`이 실제 볼트로 연결 안 됨) 3번까지 `success`로
> 보여도 볼트엔 도달 못 하는 상태였다. `n8n_launch.sh` 정식 재기동으로 해결—
> 상세는 아래 "n8n의 `/root/obsidian`이 실제 볼트에 bind mount돼 있지 않았다" 절.
> 그래도 **3번 `success`만으로 안심하지 말고 4번(실제 볼트 경로)까지 확인하는 습관은
> 유지할 것** — 이번 사고 내내 그게 유일한 확실한 확인법이었다.
...
## ✅ n8n write 단계가 사실상 한 번도 성공한 적이 없었다 (2026-08-11 사고, 해결 완료)
...
## ✅ n8n의 `/root/obsidian`이 실제 볼트에 bind mount돼 있지 않았다 (2026-08-11, 해결 완료)
...
- **최종 해결(2026-08-11 오후)**: 사용자가 별도 Termux 세션에서 기존 즉석 n8n 프로세스를
  `pkill -f "n8n start"`로 먼저 종료한 뒤 `bash ~/n8n_launch.sh`를 재실행 — 이번엔
  "이미 응답 중이라 건너뜁니다"가 아니라 실제로 proot+bind 기동 단계를 탔다(1차
  재기동 때는 기존 프로세스가 살아있어 스크립트의 idempotent 체크가 기동 자체를
  스킵했던 것이 원인). 검증 결과:
  - bind mount 정상 — `/root/obsidian`에 쓴 파일이 실제 볼트에 즉시 나타남 확인
  - 동기화 워크플로가 실제 볼트 경로에 정상 반영됨
  - 백필했던 48개 파일(2026/07 15개 + 2026/08 33개) 샘플 검증 — 프론트매터·본문 정상
  - n8n-mcp 연결 유지(셀룰러 IP 경로라 재시작 영향 없음)

**교훈 정리**: idempotent 시작 스크립트라도 "이미 떠 있음 → 건너뜀" 체크가 있으면,
**잘못된 상태로 이미 떠 있는 프로세스를 먼저 죽이지 않고 재실행해봐야 아무 효과가
없다.** 재기동 스크립트를 돌릴 땐 "떠 있는 걸 새로 띄우는 것"이 아니라 "기존 걸
내리고 다시 띄우는 것"까지가 한 세트라고 생각할 것.
