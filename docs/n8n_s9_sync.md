...
⚠️ **다이제스트 로컬 인덱스 패치와의 상호작용 (2026-08-21 발견)** —
`docs/n8n_daily_digest_workflow.json`(18:00 KST)은 로컬 `_index.md`에 다이제스트
링크를 직접 추가하는 별도 패치를 가지고 있다(`docs/n8n_daily_digest.md`의 "로컬
인덱스는 GitHub 쪽과 절대 동기화되지 않는다" 참고). 이 동기화 워크플로가 위
수정으로 매 실행 `_index.md`를 GitHub 버전으로 **통째로 덮어쓰기** 때문에, 다음날
07:10 슬롯에 그 다이제스트 링크가 사라진다(다이제스트 자체가 `3tv-reports`엔 안
올라가 있으므로 GitHub 쪽 "정식본"엔 애초에 그 링크가 없음). 즉 다이제스트
인덱스 링크는 **그날 18:00~다음날 07:10까지만** 보이는 반짝임 현상이 있다 —
영구적으로 남기려면 다이제스트 파일 자체를 `3tv-reports`에 올리는 별도 작업
(쓰기 권한 PAT 필요)이 있어야 한다. 아직 미결.
