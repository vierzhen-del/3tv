...
동작: 평일 **07:10**(us 리포트) · **08:50**(kr 병합본) · **13:00**(정오 리포트 등) · **19:00**(그날 마지막 catch-all) KST에 실행 → `3tv-reports`의 `3protv/YYYY/MM/`에서 오늘 날짜 md를 찾아 `/root/obsidian/3protv/YYYY/MM/`에 저장 → Syncthing이 1분 내 S26으로 전파.
...
### 슬롯 시각 변경 이력 — 09:40→13:00, 17:00→19:00 (2026-08-14 사용자 확정)
...
슬롯이 이걸 더 안정적으로 잡고, **19:00**은 하루의 마지막 catch-all이다. 아침 cron 지연은
07:10/08:50 사이 마진(46분 실측)이 짧지만, 그날 안에 13:00·19:00 슬롯이 남아있어 결국
잡힌다 — "오늘 날짜"만 보는 필터라 어느 슬롯이 챙기든 상관없다. 이미 받은 파일은 같은
내용으로 덮어쓰므로 슬롯이 겹쳐 중복 실행돼도 무해하다.

> ⚠️ **19:00은 다이제스트(18:00 발송) 이후입니다.** 정오 리포트가 유난히 늦게(18시 근처까지)
> 밀리는 극단적인 날엔, 13:00 슬롯이 못 잡은 리포트가 그날 다이제스트에는 안 들어가고
> 19:00 동기화로만 볼트에 반영될 수 있습니다 — 이 경우 다이제스트 재실행이나 다음날 확인이
> 필요할 수 있습니다. 사용자가 이 트레이드오프를 확인하고 이 시각들을 확정했습니다.
...
| # | 노드 | 설정 |
|---|------|------|
| 1 | Schedule Trigger | Cron: `10 7 * * 1-5`, `50 8 * * 1-5`, `0 13 * * 1-5`, `0 19 * * 1-5` (KST) |
