...
- **✅ 2026-08-13 미확인 이슈 재확인 — 08/14엔 재발 안 함**: 2026-08-13
  ...
  `볼트연동`/`운영점검` 스킬로 조사할 것.
- **VKOSPI는 항상 "확인 불가"로 뜬다(2026-08-14 확인, 버그 아님)** —
  `report.py` 프롬프트(5번 섹션)는 "국장은 VKOSPI가 검증 시세에 있으면
  사용"이라고 LLM에 지시하지만, **VKOSPI를 조회하는 코드가 어디에도 없다**
  (`config/settings.yaml`의 `indices` 목록에도, `market.py`에도 티커·구현이
  전혀 없음 — 코스피200 선물처럼 "yfinance/pykrx에 없어 방송화면 vision으로
  대체" 같은 우회 방법도 마련돼 있지 않다). 그래서 매일 "확인 불가"로
  적히는 게 정상 동작이다 — 데이터 소스가 원천적으로 없는 상태. pykrx가
  KRX 자체 지수 코드로 VKOSPI를 지원하는지는 이 세션에서 네트워크 제약으로
  실측 못 함(KRX 직접 호출이 프록시에서 막힘). 매일 뜨는 "확인 불가"가
  거슬리면 ① pykrx로 실제 구현 시도(먼저 KRX 인덱스 코드 확인 필요) 또는
  ② 프롬프트에서 VKOSPI 요청 자체를 빼는 것 중 선택 필요 — 사용자 확인 대기.
