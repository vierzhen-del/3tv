...
- **다이제스트 구성(2026-08-13 확장, 코드 추출 방식으로 재설계)**: 4개 섹션 —
  ...
  - **①·②는 Gemini가 아니라 "다이제스트용 본문 구성" Code 노드가 정규식으로
    ...
    - ⚠️ **정규식은 옵시디안 아카이브본의 실제 마크다운 형식**을 따라야
      ...
      `**수급주체 요약 (YYYYMMDD 기준)**` 헤더 + `*   개인/외국인/기관합계:
...
- **🔴 미확인 이슈(2026-08-13 발견, 원인 미조사)**: 2026-08-13 "3protv오늘" 리포트를
  열어보니 `<!-- 3tv:us -->` 미국 시황 섹션만 있고 국내(kr) 시황 섹션이 아예
  없었다 — KOSPI/KOSDAQ/코스피 선물 데이터가 리포트 자체에 없는 상태였다.
  다이제스트 쪽 버그가 아니라 **그날 kr 세션이 us 리포트에 정상 병합됐는지
  자체를 의심해야 하는 문제**로 보인다. `볼트연동`/`운영점검` 스킬로 원인
  조사가 필요 — kr-session Actions 로그부터 확인할 것.
