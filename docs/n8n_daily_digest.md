...
- **다이제스트 구성(2026-08-13 확장, 코드 추출 방식으로 재설계)**: 5개 섹션 —
  ① 📊 주요 지수(다우/S&P500/나스닥/코스피/코스닥) 전일 대비, ② 💰 수급 동향
  (개인/외국인/기관, 전일대비), ③ 🌙 코스피 선물 전일 대비, ④ 📈 리포트에
  언급된 종목 마감 전일 대비, ⑤ 📋 오늘 흐름 3~5줄 요약.
  ...
  - **③은 코드 추출이 안 됩니다** — 코스피(200) 선물은 yfinance/pykrx에
    API가 없어서 `_quote_line()` 같은 정형 포맷이 아니라 **방송 화면을
    vision으로 읽은 값**이 리포트 본문 문장 속에 섞여 들어갑니다
    (`report.py` 1249줄 근처 지시문 참고). 그래서 ③·④는 여전히 Gemini가
    본문에서 찾습니다. 코스피 선물은 방송에 안 나온 날은 리포트에 아예
    없을 수 있어 "데이터 없음"이 정상적으로 뜰 수 있습니다.
  - 리포트에 없는 지수/수급/코스피선물/종목은 각각 "데이터 없음"/"언급 종목
    없음"으로 표기됩니다.
